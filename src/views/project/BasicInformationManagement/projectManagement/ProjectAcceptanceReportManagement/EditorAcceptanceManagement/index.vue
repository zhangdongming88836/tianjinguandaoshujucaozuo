<template>
  <div id="EditDocumentManagement">
    <el-dialog title="编辑" :visible.sync="ProjectManagementEditor" width="40%" center @open="open" @close="close">
...
export default {
  ...
  methods: {
    ...
    async open() {
      this.UploadedFiles = []
      await acceptanceReportProject().then(res => {
        // console.log("dakdoak")
        // console.log(res,"/*/*/*/*/*")
        this.projectNames = res.data.data
      })
      await acceptanceReportDetails(this.ruleForm.id).then(res => {
        console.log(res, '获取编辑数据')
        this.ruleForm.projectName = res.data.data.ar.projectName
        this.ruleForm.stage = res.data.data.ar.stage.split(',')
        this.ruleForm.projectId = res.data.data.ar.projectId
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          let obj = {
            ...
            stage: srt,
            attachments: this.attachments == {} ? null : this.attachments, //this.ruleForm.fileName[0],
            id: this.ruleForm.id,
            ids: this.ids,
          }
          acceptanceReportUpdate(obj).then(res => {
