          ...
          <template v-for="item in titles">
            ...
            >
              <template slot-scope="scope">
                <a v-if="scope.row.fileType == 'pdf'" :href="scope.row.fileUrl" target="_blank" style="cursor: pointer">预览</a>
                <a v-else @click="UnableToClick">预览</a>
                <el-button type="text" size="mini" @click="SingleDownload(scope.row)">下载</el-button>
              </template>
            </el-table-column>
            <el-table-column
              v-else
...
export default {
  props: ['DocumentForm', 'reportCode', 'GprojectName', 'gjrows'],
  data() {
    return {
      GprojectName: '',
      reportCode: '',
    ...
    SingleDownload(val) {
      ...
      let link = document.createElement('a')
          ...
          link.href = `${val.fileUrl}`
           link.setAttribute('download', val.documentName) // 自定义下载文件名（如exemple.txt）
          link.download=val.documentName
  ...
  watch: {
    ...
    multipleSelection: function (newVal, oldVal) {
      this.$emit('GJUploadClose', newVal)
    },
  },
  created() {
    let obj = {
